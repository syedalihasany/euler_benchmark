...
#define __restrict 
#endif
std::ofstream featurefile ("input_features.csv", std::ios::out); 			// file to collect input features
std::ofstream targetfile ("output_features.csv", std::ios::out);			// file to collect output features
...
void compute_step_factor(int nelr, float* __restrict variables, float* areas, float* __restrict step_factors, int k) //int k is added for data collection logic
{
	// creating two text files: one to collect input data and the other to collect output data
	//std::ofstream featurefile ("input_features.txt", std::ios::out); 			// file to collect input features
	//std::ofstream targetfile ("output_features.txt", std::ios::out);			// file to collect output features

	if ( !featurefile.is_open() ) {												// checking input file handle
		std::cout<<"Unable to open the input feature file"<<std::endl;
		
		}

	if ( !targetfile.is_open() ) {												// checking output file handle
...
#pragma omp simd
	for(int i = b_start; i < b_end; i++)
	{
		...
		if ( k > 15 ) { // if condition added to collect data after the 15th iteration
		featurefile << density << "," << density_energy << ","<< momentum.x << "," << momentum.y << "," << momentum.z << ","  << pressure << "," << areas[i] << std::endl;
		...
		targetfile << step_factors[i] << std::endl;
		}
...
int main(int argc, char** argv)
{
	...
	for(int i = 0; i < iterations; i++)
	{
		...
		compute_step_factor(nelr, variables, areas, step_factors, i);
