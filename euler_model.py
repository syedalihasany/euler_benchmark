...
class MLPModel(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc3 = nn.Linear(32, 64)  # Added FC layer
        self.fc4 = nn.Linear(64, 128)  # Added FC layer
        self.fc5 = nn.Linear(128,512)  # Added FC layer
        self.fc6 = nn.Linear(512, 1024)
        self.fc7 = nn.Linear(1024, 50)
        self.fc8 = nn.Linear(50, 20)
        self.fc9 = nn.Linear(20, output_size)
    ...
    def forward(self, x):
        ...
        x = self.fc2(x)
        x = self.relu(x)
        x = self.fc3(x)  # Added FC layer
        x = self.relu(x)
        x = self.fc4(x)  # Added FC layer
        x = self.relu(x)
        x = self.fc5(x)  # Added FC layer
        x = self.relu(x)
        x = self.fc6(x)
        x = self.relu(x)
        x = self.fc7(x)
