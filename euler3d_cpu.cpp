 ...
 */
#define GAMMA 1.4
#define iterations 1000				// changed number of iterations from 2000 to 1000
...
#pragma omp simd
	for(int i = b_start; i < b_end; i++)
	{
		...
		// defining three floats to get values for the momentum struct that is not accessible directly
		float momentum_x = momentum.x;
		float momentum_y = momentum.y;
		float momentum_z = momentum.z;
		

		float density_energy = variables[i + VAR_DENSITY_ENERGY*nelr];
		float3 velocity;	   compute_velocity(density, momentum, velocity);

		// defining three floats to get values for the velocity struct that is not accessible directly
		...
		featurefile << density << "\t" << density_energy << "\t"   \
         << momentum_x << "\t" << momentum_y << "\t" << momentum_z << "\t" << velocity_x << "\t" \
        << velocity_y << "\t" << velocity_z << "\t" << areas[i] << std::endl;
